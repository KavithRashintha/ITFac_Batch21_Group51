...
Feature: Plant Management UI - User
  ...
  Scenario: Verify filter by a specific plant category
    Then User selects category "Sub_Cat1" from filter
    And User clicks "Search" button
    And Only plants with category "Sub_Cat1" should be displayed

  Scenario: Verify search plants by existing plant name
    Then User searches for plant "Plant 2"
    And The plant "Plant 2" should be visible in the results
  ...
  Scenario: Verify search with a valid plant name and a specific category filter
    ...
    And User selects category "Sub_Cat1" from filter
