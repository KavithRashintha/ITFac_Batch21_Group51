...
When(
  "user clicks on edit button for category {string}",
  async function (oldCategoryName) {
    const categoryPage = new CategoryPage(this.page);
    const row = this.page
      .locator("table tbody tr")
      .filter({ hasText: oldCategoryName })
      .first();
...
Then(
  "the category name should be updated to {string}",
...
Then(
  "the category name should remain as {string}",
...
When(
  "user clicks on delete button for category {string}",
...
Then(
  "Add Category button should be visible and enabled",
  { timeout: 15000 },
  async function () {
    const categoryPage = new CategoryPage(this.page);
...
When(
  "user clicks save button without entering category name",
  async function () {
    const categoryPage = new CategoryPage(this.page);
    await this.page.click(categoryPage.saveButton);
  },
...
Then("user sorts category list by ID", async function () {
  ...
  const getIds = async () =>
    (await this.page.locator("tbody tr td:first-child").allTextContents()).map(
      (id) => Number(id.trim()),
    );
...
Then(
  ...
  async function (parentCategory) {
    ...
    const nonEmptyParents = parentColumnValues.filter(
      (p) => p.trim() && p.trim() !== "-",
    );
