...
// ===================== Filter by category =====================
When(
  "User selects category {string} from filter",
  { timeout: 15000 },
...
// ===================== Search plants =====================
When(
  "User searches for plant {string}",
  { timeout: 20000 },
...
Then(
  "The plant {string} should be visible in the results",
  { timeout: 10000 },
  async function (plantName) {
...
Then(
  "Plants with quantity below {int} should display the {string} badge",
  { timeout: 60000 },
...
When("User clicks on {string} column header", async function (columnName) {
  const byRole = this.page.getByRole("columnheader", {
    name: columnName,
    exact: false,
  });
  await byRole.click();
...
Then(
  "User see the sort indicator {string} in the name column",
  async function (direction) {
    ...
    const textIndicator = header
      .locator("span")
      .filter({ hasText: expectedChar });
