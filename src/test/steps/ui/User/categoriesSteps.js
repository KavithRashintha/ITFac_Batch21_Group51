...
Then(
  "Search results should be displayed in categories table",
  async function () {
    const categoryPage = new CategoryPage(this.page);
...
Then(
  "Results should match the search criteria in categories list",
  async function () {
    const categoryPage = new CategoryPage(this.page);
...
When(
  "User selects parent category {string} from dropdown",
...
Then(
  "Only subcategories belonging to parent category {string} should be visible",
  async function (category) {
    await this.page.waitForLoadState("networkidle");
